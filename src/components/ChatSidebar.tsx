...
const ChatSidebar: React.FC<ChatSidebarProps> = ({ isOpen, onClose, chat }) => {
  // Add body styling when sidebar is open to prevent horizontal overflow
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-40 w-[400px] sm:w-[540px] bg-black/20" onClick={onClose}>
      <div 
        className="absolute inset-y-0 right-0 w-[400px] sm:w-[540px] bg-background shadow-xl h-full"
